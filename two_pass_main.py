...
gemini_flagger = gemini_processor.GeminiProcessor(
    model_name="gemini-2.0-flash",
    temperature=0.4,
    api_key=os.getenv("GEMINI_API_KEY"),
...
def loop_local_files(
    ...
    flag_prompt_path: str = "prompt_engineering_flagforcleaning.md",
    eda_prompt_path: str = "prompt_engineering_parsing.md",
    ...
    try:
        flagger_prompt = gemini_flagger.load_prompt_template(flag_prompt_path)
        eda_prompt = gemini.load_prompt_template(eda_prompt_path)
    ...
    for filename in os.listdir(loop_dir):
        ...
        try:
            ...
            gemini_flagger.upload_file(processed_file_path)

            models_data = []
            # Pass 1: Flag for Cleaning (using a cheaper, faster model)
            logger.info("Running Pass 1: Flagging for cleaning...")

            flag_response = gemini_flagger.generate_content(prompt=flagger_prompt)
            cleaning_data = _parse_llm_text_to_dict(flag_response.text, gemini_flagger.file_name)

            models_data.append({
                "model_type" : "Cleaning flagger",
                "model_name": gemini_flagger.model_name,
                "usage_tokens": flag_response.usage_metadata.total_token_count
            })


            # Pass 2: EDA (using a more powerful model)
            logger.info("Running Pass 2: Extracting EDA data...")
            gemini.uploaded_resume_file = gemini_flagger.uploaded_resume_file
            eda_response = gemini.generate_content(prompt=eda_prompt)
            eda_data = _parse_llm_text_to_dict(eda_response.text, gemini_flagger.file_name)
            models_data.append({
                "model_type" : "EDA",
                "model_name": gemini.model_name,
                "usage_tokens": eda_response.usage_metadata.total_token_count
            })
